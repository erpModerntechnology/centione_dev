...
class CrmLead(models.Model):
    ...
    def action_open_leads(self):
        ...
        return action
    def action_amed_leads(self):
        """ Open meeting's calendar view to schedule meeting on current opportunity.
            :return dict: dictionary value for created Meeting view
        """
        self.ensure_one()
        res_ids = self.env['res.reservation'].search([('lead_id', '=', self.id),('custom_type','=','Accessories')])
        action = self.env["ir.actions.actions"]._for_xml_id("mabany_real_estate.reservation_list_action")
        action['context'] = {
            'default_lead_id': self.id,
        }
    ...
    count_amed = fields.Integer(compute='count_amed_recs')
    ...
    def count_amed_recs(self):
        for rec in self:
            count = self.env['res.reservation'].search_count([('lead_id', '=', rec.id),('custom_type','=','Accessories')])
            rec.count_amed = count
