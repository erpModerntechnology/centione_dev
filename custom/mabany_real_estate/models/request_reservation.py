...
class requestReservation(models.Model):
    ...
    _description = "Request Unit Reservation"
    ...
    def create_reservation(self):
        res_res = self.env['res.reservation'].search([('req_reservation_id', '=', self.id),('property_id', '=', self.property_id.id),
                                                      ('state', 'in', ['reserved'])])
        if len(res_res) != 0:
            raise ValidationError(_("Sorry .. you must Create One Reservation Form For Request Reservation for This Unit  %s!!")% self.property_id.name)
        lines=[]
    ...
    def generate_report(self):
        ...
        return self.env.ref('mabany_real_estate.payment_request_report_id').report_action([], data=data)
