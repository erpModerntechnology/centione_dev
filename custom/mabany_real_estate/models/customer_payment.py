...
class Customer_payment(models.Model):
    ...
    journal_id = fields.Many2one(comodel_name="account.journal", string="Payment Method to", required=True,domain=[('type', 'in', ['bank','cash'])],  )
    partner_id = fields.Many2one(comodel_name="res.partner", string="Partner", required=True, )
    reservation_id = fields.Many2one(comodel_name="res.reservation", string="Reservation", )
    rent_id = fields.Many2one(comodel_name="res.rent", string="Rent", )
    ...
    type_id = fields.Selection([('rent', 'Rent'),
    ...
    customer_ids = fields.Many2many(comodel_name="res.partner", relation="res_partner_reservation_2", column1="par2", column2="reservation2", string="", )
    ...
    def onchange_reservation_id(self):
        if self.reservation_id:
            ...
            self.loan_line = [(6,0,[])]
            self.customer_ids = [(6,0,self.reservation_id.customer_ids.ids)]
        ...
        if self.is_select_all:
            for line in self.loan_line:
                 ...
                 if line.amount > line.amount_due:
                    raise UserError(_("You Cant Payment bigger than amount due."))
                 if line.type == 'bank':
                 ...
                 ref = ''
                 if self.reservation_id:
                     ref = self.name + " -> " + self.reservation_id.name
                 if self.rent_id:
                     ref = self.name + " -> " + self.rent_id.name
                 ...
                 pay = account_payment.create({
                     ...
                     'customer_ids': [(6,0,self.reservation_id.customer_ids.ids)],

                 })

                 data_bank_id = data_bank.create({
                     'bank_id': line.bank_name.id,
                     'cheque_number': str(line.cheque),
                     'reservation_id': self.reservation_id.id,
                     'payment_id': pay.id,
            ...
            for line in self.loan_line:
             if line.type == 'bank':
                 ...
                 method = self.env['account.payment.method'].sudo().search([('name', '=', 'Batch Deposit')], limit=1)

                 if not line.bank_name.id and line.is_pay:
                     raise UserError(_("You Must Add Bank Name."))
                 if not line.cheque and line.is_pay:
                     raise UserError(_("You Must Add Cheque Number."))
             ...
             if line.is_pay:
                 ...
                 print("pay kero 2 :: ",line.installment_line_id)
                 pay_strg = self.env['payment.strg'].sudo().search([
                     ('id', '=', line.installment_line_id)
                 ], limit=1)
                 print("pay kero 2 :: ",line.bank_name.id)
                 print("pay kero 2 :: ",line.cheque)
                 print("pay kero 2 :: ",self.reservation_id.id)
                 print("pay kero 2 222:: ",pay2.id)
                 ...
                 if pay2:
                    pay2.payment_method_id = method.id
                    pay2.payment_method_line_id = self.env['account.payment.method.line'].sudo().search([('name', '=', 'Batch Deposit')], limit=1).id
