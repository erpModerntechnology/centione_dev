...
class BatchDepositChecks(models.Model):
    ...
    def _compute_state(self):
        for batch in self:
            if batch.payment_method_id.id != 5:
    ...
    #             payment_method_obj = self.env['account.payment.method']
    #             payment = payment_method_obj.search([('code', '=', 'batch_payment')], limit=1)
    #             batch.payment_method_id = payment.id
    #         else:
    #             batch.payment_method_id = False
    ...
    def post_under_collection(self):
        for r in self.payment_ids:
            ...
            if r.state_check == 'posted' or r.state_check == 'refunded_under_collection':
                r.post(date_under_collect=r.date)
    ...
    def change_state_refunded_and_collect(self):
        for r in self.payment_ids:
            if r.state_check != "collected":
                if r.state_check != "refunded_under_collection":
                    return False
        return True
    ...
    def discount_all(self):
        ...
        if self.state == 'draft':
            ...
            if from_wizard:
                ...
                for r in self.payment_ids:
                    if r.state_check in ['posted', 'refund_from_discount', 'refunded_under_collection']:
    ...
    def receive_all(self):
        ...
        for r in self.payment_ids:
            ...
            if r.state_check not in ['discount', 'refund_from_discount'] and r.multi_select:
                raise ValidationError(
                    'you try to loan check number {} and it\'s not in discount or refunded from discount stages'.format(
                        r.check_number_2))
            if r.state_check == 'loan':
                raise ValidationError(
                    'you try to loan check number {} and it\'s already loaned'.format(
                        r.check_number_2))

            if not r.loan_date and r.multi_select:
                raise ValidationError('please enter loan date for check number {}'.format(r.check_number_2))
            if r.state_check == 'discount' and r.multi_select:
                r.post()
                r.multi_select = False
                no_select = False
            if r.multi_select:
                no_select = False
            if r.state_check not in ['collected', 'refund_from_discount']:
    ...
    def refund_discount(self):
        for rec in self:
            ...
            for r in rec.payment_ids:
                ...
                if r.state_check != 'loan' and r.multi_select:
                    raise ValidationError(
                        'you try to refund check number {} and it\'s not in loan stage'.format(r.check_number_2))

                if not r.ref_coll_batch and r.multi_select:
                    raise ValidationError(
                        'please enter [Refund/collect] date for check number {}'.format(r.check_number_2))

                if r.multi_select == True and r.state_check in ['loan', 'discount']:
                    r.refund_discount()
                    r.multi_select = False
                    no_selection = False
    ...
    def collect_discount(self):
        ...
        for r in self.payment_ids:
            ...
            if r.state_check == 'collected' and r.multi_select:
                raise ValidationError(
                    'you try to collect check number {} and it\'s already collected'.format(
                        r.check_number_2))
            if r.state_check != 'loan' and r.multi_select:
                raise ValidationError(
                    'you try to collect check number {} and it\'s not in loan stage'.format(r.check_number_2))

            if not r.ref_coll_batch and r.multi_select:
                raise ValidationError('please enter [Refund/collect] date for check number {}'.format(r.check_number_2))
        ...
        if line:
            ...
            if max_available < used_with_amount:
                msg += "- amount for check Number ({}) to partner ({}) is exceed the central percentage in bank ({}) \n".format(
                    line.check_number_2, line.partner_id.name, self.bank_id.bank_id.name)
            if len(msg) > 2:
                return msg
            else:
                False
            ...
            for p in partner:
                ...
                for r in self.payment_ids:
                    ...
                    check_number_2s += r.check_number_2 + ","
                if max_available < used_with_amount:
                    exceed = True
                ...
                if exceed:
                    if len(self.payment_ids) > 1:
                        msg += "- Total amount for check Number's ({}) to partner ({}) is ({}) exceed the central percentage ({}) in bank ({})\n" \
                            ...
                            check_number_2s[:-1], p.name, used_with_amount, max_available, self.bank_id.bank_id.name
        ...
        if line:
            ...
            if available_pool < used_with_amount:
                msg += "- amount for check Number ({}) is exceed the available pool in bank ({}) \n".format(
                    line.check_number_2, self.bank_id.bank_id.name)
            if len(msg) > 2:
