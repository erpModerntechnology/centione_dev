...
odoo.define('portal_excuse', function(require) {
    'use strict';
...
//            console.log(num_of_days);
        ...
        var excuses_headers = $('.o_excuses_management_portal_users thead .o_list_record_selector input');
        var excuses_rows = $('.o_excuses_management_portal_users tbody .o_list_record_selector input');
        excuses_rows .prop('checked', false);
        excuses_headers .prop('checked', false);
        excuses_headers.click(function () {
            if (!$(this).prop('checked')) {
                $('#button_to_remove_excuses').addClass('o_hidden');
            } else {
                $('#button_to_remove_excuses').removeClass('o_hidden');
            }
            excuses_rows.prop('checked', $(this).prop('checked') || false);
        });
        excuses_rows.click(function () {
            if (excuses_headers.prop('checked') && !$(this).prop('checked')) {
                excuses_headers.prop('checked', false);
            }
            var something_checked = false;
            var i;
            for (i=0; i<excuses_rows.length; i++) {
                if (excuses_rows[i].checked) {
        ...
        $('#delete_excuse_id').click(function () {
            if ( ! confirm(_t('Do you really want to remove these records?'))) {
                event.preventDefault();
            } else {
        ...
        $('.o_excuses_management_portal_users').on('change', "select[name='start_date_half_day']", function (e) {
