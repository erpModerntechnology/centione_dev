...
odoo.define('portal_termination', function(require) {
    'use strict';
...
//            console.log(num_of_days);
        ...
        var terminations_headers = $('.o_terminations_management_portal_users thead .o_list_record_selector input');
        var terminations_rows = $('.o_terminations_management_portal_users tbody .o_list_record_selector input');
        terminations_rows .prop('checked', false);
        terminations_headers .prop('checked', false);
        terminations_headers.click(function () {
            if (!$(this).prop('checked')) {
                $('#button_to_remove_terminations').addClass('o_hidden');
            } else {
                $('#button_to_remove_terminations').removeClass('o_hidden');
            }
            terminations_rows.prop('checked', $(this).prop('checked') || false);
        });
        terminations_rows.click(function () {
            if (terminations_headers.prop('checked') && !$(this).prop('checked')) {
                terminations_headers.prop('checked', false);
            }
            var something_checked = false;
            var i;
            for (i=0; i<terminations_rows.length; i++) {
                if (terminations_rows[i].checked) {
        ...
        $('#delete_termination_id').click(function () {
            if ( ! confirm(_t('Do you really want to remove these records?'))) {
                event.preventDefault();
            } else {
        ...
        $('.o_terminations_management_portal_users').on('change', "select[name='start_date_half_day']", function (e) {
