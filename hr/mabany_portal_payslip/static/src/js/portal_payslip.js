...
odoo.define('portal_payslip', function(require) {
    'use strict';
...
//            console.log(num_of_days);
        ...
        var payslips_headers = $('.o_payslips_management_portal_users thead .o_list_record_selector input');
        var payslips_rows = $('.o_payslips_management_portal_users tbody .o_list_record_selector input');
        payslips_rows .prop('checked', false);
        payslips_headers .prop('checked', false);
        payslips_headers.click(function () {
            if (!$(this).prop('checked')) {
                $('#button_to_remove_payslips').addClass('o_hidden');
            } else {
                $('#button_to_remove_payslips').removeClass('o_hidden');
            }
            payslips_rows.prop('checked', $(this).prop('checked') || false);
        });
        payslips_rows.click(function () {
            if (payslips_headers.prop('checked') && !$(this).prop('checked')) {
                payslips_headers.prop('checked', false);
            }
            var something_checked = false;
            var i;
            for (i=0; i<payslips_rows.length; i++) {
                if (payslips_rows[i].checked) {
        ...
        $('#delete_payslip_id').click(function () {
            if ( ! confirm(_t('Do you really want to remove these records?'))) {
                event.preventDefault();
            } else {
        ...
        $('.o_payslips_management_portal_users').on('change', "select[name='start_date_half_day']", function (e) {
