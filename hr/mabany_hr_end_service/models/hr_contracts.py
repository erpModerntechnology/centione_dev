...
class HrContract(models.Model):
    ...
    def get_end_of_service(self):
            if (self.date_end and self.date_start) and (self.date_end > self.date_start):
                if self.env["ir.config_parameter"].sudo().get_param("is_calculated"):
    ...
    def end_of_contract_notification(self):
        ...
        users = self.env.ref('mabany_hr_end_service.group_end_of_service_notifications').users
        ...
        for contract in contracts:
            notification_temp = self.env.ref('mabany_hr_end_service.mail_template_end_of_contract')
