...
class EndServiceIncentive(models.TransientModel):
    ...
    end_incentive_month = fields.Float(string="End of Service Months",default=lambda self:self.env["ir.config_parameter"].sudo().get_param("end_service_incentive"))
    is_incentive_calc = fields.Boolean(string="Is Incentive Calc",default=lambda self:self.env["ir.config_parameter"].sudo().get_param("is_calculated") )
