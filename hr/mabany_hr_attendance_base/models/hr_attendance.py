...
class Attendance(models.Model):
        ...
        for check in checks:
            if check.check_out:
                ...
                if (0 < period_in_days < 1) and in_date.day == out_date.day:
                    ...
                    # raise exceptions.ValidationError(_("Check-in record is not prior to the Check-out record.")) #omara stopped
                    return result
    ...
    def generate_scheduled_workdays(self, work_schedule, date_from_str, date_to_str):
        ...
        date_from = datetime.strptime(str(date_from_str), DEFAULT_SERVER_DATE_FORMAT)
        date_to = datetime.strptime(str(date_to_str), DEFAULT_SERVER_DATE_FORMAT)
    ...
    def analyze_attendance(self, employee, work_schedule, date_from_str, date_to_str, include_leave_types=None,
        ...
        for day in scheduled_workdays:
            ...
            ##omara added 14 jan2021 for set timezone
            day=day.replace(tzinfo=pytz.timezone('Egypt'))

            ##omara added
            workday_intervals = work_schedule._work_intervals(day,day)
            ...
            #omara added 21 jan2021
            if not workday_intervals:
                return []
            #omara
            _required_working_hours = (workday_intervals[0].end_datetime - workday_intervals[
                0].start_datetime).seconds / 3600
            if _attendance_intervals:
                if _attendance_intervals[0][1] < _required_working_hours:
                    _check_in = _attendance_intervals[0][2]
                    _supposed_check_out = min(_check_in + timedelta(hours=_required_working_hours),
                                              workday_intervals[0].end_datetime + timedelta(hours=work_schedule.flexible_hours))
                    workday_intervals[0] = self.env['resource.calendar']._interval_new(workday_intervals[0][0],
                                                                                       _supposed_check_out,
                                                                                       workday_intervals[0].data
                                                                                       )
    ...
    def compute_diff_intervals(self, actual_intervals, scheduled_intervals, employee_leaves):
        ...
        if scheduled_intervals[0][2]['attendances'].type == 'flexible':
            flexible_minutes = scheduled_intervals[0][2]['attendances'].flexible_hours * 60
        else:
            flexible_minutes = 0.0
        ...
        for sub_interval in sub_intervals:
            ...
            if nesting_actual_interval and not nesting_scheduled_interval and not nesting_leave_interval:
                ...
                analyzed_interval = AnalyzedInterval(N_VE, sub_interval)
                if analyzed_interval.period_in_minutes > flexible_minutes and flexible_minutes:
                    analyzed_interval.period_in_minutes -= flexible_minutes
                    flexible_minutes = 0.0
                    diff_intervals.append(analyzed_interval)
                else:
                    if flexible_minutes:
                        flexible_minutes -= analyzed_interval.period_in_minutes
                        sub_interval[0] + time.timedelta(minutes=analyzed_interval.period_in_minutes)
                        diff_intervals.append(AnalyzedInterval(P_VE, sub_interval))
                    else:
                        diff_intervals.append(analyzed_interval)
