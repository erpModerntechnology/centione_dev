...
odoo.define('portal_employee', function(require) {
    'use strict';
...
//            console.log(num_of_days);
        ...
        var employees_headers = $('.o_employees_management_portal_users thead .o_list_record_selector input');
        var employees_rows = $('.o_employees_management_portal_users tbody .o_list_record_selector input');
        employees_rows .prop('checked', false);
        employees_headers .prop('checked', false);
        employees_headers.click(function () {
            if (!$(this).prop('checked')) {
                $('#button_to_remove_employees').addClass('o_hidden');
            } else {
                $('#button_to_remove_employees').removeClass('o_hidden');
            }
            employees_rows.prop('checked', $(this).prop('checked') || false);
        });
        employees_rows.click(function () {
            if (employees_headers.prop('checked') && !$(this).prop('checked')) {
                employees_headers.prop('checked', false);
            }
            var something_checked = false;
            var i;
            for (i=0; i<employees_rows.length; i++) {
                if (employees_rows[i].checked) {
        ...
        $('#delete_employee_id').click(function () {
            if ( ! confirm(_t('Do you really want to remove these records?'))) {
                event.preventDefault();
            } else {
        ...
        $('.o_employees_management_portal_users').on('change', "select[name='start_date_half_day']", function (e) {
